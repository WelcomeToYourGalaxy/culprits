...
function syncGroupBox(box) {
  const parent = box.querySelector("[data-group]");
  if (!parent) return;
  const on = CT_HISTORY.children.filter(
    (c) => (visibility.get(c.id) || "none") === "visible").length;
  parent.checked = on === CT_HISTORY.children.length && on > 0;
  parent.indeterminate = on > 0 && on < CT_HISTORY.children.length;
  const el = box.querySelector(`[data-state="${CT_HISTORY.id}"]`);
  if (el) {
    el.textContent = on === 0
      ? `${CT_HISTORY.children.length} year archives, loaded on demand`
      : `${on} of ${CT_HISTORY.children.length} years showing`;
  }
}
...
function buildPanel() {
  ...
  // nothing.
  if (CT_HISTORY.children.length) box.appendChild(groupRows(CT_HISTORY));
  ...
  box.addEventListener("change", (e) => {
    ...
    if (e.target.dataset.group) {
      ...
      CT_HISTORY.children.forEach((child) => {
    ...
    if (e.target.checked) {
      const child = CT_HISTORY.children.find((c) => c.id === id);
