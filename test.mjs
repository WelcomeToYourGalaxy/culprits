...
// --- layer groups ----------------------------------------------------------
{
  ...
  check("the group renders only when it has children",
        /if \(CT_HISTORY\.children\.length\) box\.appendChild/.test(src));
  ...
  check("the parent is only checked when every child is on",
        /checked = on === CT_HISTORY\.children\.length/.test(sync));
